...
class OmnitabTokenizer(TapexTokenizer):
    ...
    def truncate_table(
        ...
        for tab, mtl in zip(tables, max_table_lengths):
            ...
            # remove rows
            self.truncate_header_rows_at_bottom(tab_content, max_length=mtl)
    ...
    def truncate_header_rows_at_bottom(
        self, 
        table_content: Dict, 
        max_length: int
    ):
        # TODO: in the original OmniTab code truncation is performed at cell level
    ...
    def _encode_plus(
        ...
        **kwargs
        ...
        token_ids, masked_token_ids, replace_unmasked_tokens_with_pad = self.perform_masking(token_ids, valid_mask_spans, **(mask_kwargs or {}))
        ...
        if replace_unmasked_tokens_with_pad:  # convert added special tokens to pad
            is_list = False
            if type(target['input_ids']) is list:
                is_list = True
                target['input_ids'] = torch.tensor(target['input_ids'])
                target['special_tokens_mask'] = torch.tensor(target['special_tokens_mask'])
            target['input_ids'][target['special_tokens_mask'].eq(1)] = self.pad_token_id
            if is_list:
                target['input_ids'] = target['input_ids'].tolist()
        source['label_input_ids'] = target['input_ids']
        return source
    ...
    def _batch_prepare_for_model(
        ...
        **kwargs
        ...
        has_labels = False
        if 'label_input_ids' in batch_outputs:
            has_labels = True
            labels = batch_outputs['label_input_ids']
        ...
        if has_labels:
            ...
            for k, v in labels_batch_outputs.items():
        ...
        if replace_unmasked_tokens_with_pad:  # replace unmasked tokens with the pad token
            for i in range(len(token_ids)):
                if i not in masked_positions:
                    token_ids[i] = self.pad_token_id

        assert len(masked_token_ids) <= len(token_ids), 'masking caused an unexpected increasing of #tokens'
        return token_ids, masked_token_ids, replace_unmasked_tokens_with_pad
